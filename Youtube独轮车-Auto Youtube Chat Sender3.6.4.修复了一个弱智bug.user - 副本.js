...

{
  ...
  const config = {
    ...
    autoDownVoteCurrChannel: "", // 开始自动点踩的账号id
    minDownVoteMin: 100, // 最短点踩挂机时间
    maxDownVoteMin: 200, // 最长点踩挂机时间
    minChangeChannel: 20, // 换频道前最短挂机时间
    maxChangeChannel: 30, // 换频道前最长挂机时间
    autoAttack1: false, // 换号等待期间是否自动开火
    cycleCheck: true, // 点完一圈后是否循环补踩
    autoSCCheck: false, // 是否开启自动sc检测
    url: "", // 当前url

    control: false, // 开启泥头登陆舰远程控制模式
  ...
  const pressDownVote = (checkTSKK = true) => {
    ...
    if (!checkTSKK || channelName === "Coco Ch. 桐生ココ") {
  ...
  const getJSON = () => {
    ...
    try {
      ...
      return {
        version: "3.6.3",
        logs: "3.6.3: 现在可以关闭手滑复制进来的自动sc检测功能了;3.6.2: 修复了在不该跳转的页面乱跳转的bug;3.6.1: 添加了独轮车游而不击的自动点踩策略, ui外观优化; 3.6.0: 穿甲弹回归; 3.5.0: 修复了无视用户设置进行去半角弹药精制的bug，程序员已拉出去祭天",
        announcement: "未加载云公告",
  ...
  const changeCurrChannel = (target, delay) => {
    const { autoDownVote, cycleCheck, autoDownVoteCurrChannel } = config;
    console.log(`delay: ${delay}`);
    if (!cycleCheck) {
      const targetName = target.children[0].children[1].children[0].innerText;
      console.log(`target channel name: ${targetName}`);
      if (targetName === autoDownVoteCurrChannel) {
        return;
      }
    }
    if (autoDownVote) {
  ...
  const delayDownVote = () => {
    ...
    if (autoAttack1 && refs.runBtn.innerText === "出动") {
      refs.runBtn.click();
    }
    const downVoteBtn = document.getElementById("top-level-buttons").children[1];
    if (downVoteBtn.getElementsByTagName("button")[0].getAttribute("aria-pressed") === "false") {
      // 还没点
      console.log("这个频道还没点过踩");
      delayDownVoteTimer = setTimeout(() => {
        if (autoDownVote) {
          pressDownVote(false);
          changeCurrChannel(accountInfomation.nextChannel, randomInt(5000, 15000));
        }
      }, randomInt(minDownVoteMin * 1000, Math.max(minDownVoteMin, maxDownVoteMin) * 1000));
    } else {
      // 点过了
      console.log("这个频道点过踩了");
      changeCurrChannel(
        accountInfomation.nextChannel,
        randomInt(minChangeChannel * 1000, Math.max(minChangeChannel, maxChangeChannel) * 1000),
      );
    }
  };

  // 打开个人信息
  const setUpAccountInfo = (runAutoDownVote = true, setupCurrAccount = false) => {
    const { control, autoDownVote, url } = config;
    ...
    // 锁死在开火直播间
    if (!control && !urlEqual(window.location.href, url)) {
      console.log("直播间不对");
      return;
    }
    let setuped = false;
    const accounts = document.getElementsByTagName("ytd-account-item-renderer");
    if (accounts.length > 0) {
      ...
      for (let i = 0; i < accounts.length; i++) {
        ...
        const accountName = currAccount.children[0].children[1].children[0].innerText;
        if (!currAccount.children[0].children[2].hidden) {
          accountInfomation.inited = true;
          accountInfomation.currChannel = accountName;
          accountInfomation.nextChannel = accounts[(i + 1) % accounts.length];
          accountInfomation.email = currAccount.parentNode.parentNode.children[0].innerText;
          accountInfomation.addAccountButton = currAccount.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[1].children[0].children[1].children[0].children[0];
          break;
        }
      }
      accounts = currAccount.parentNode.children;
      for (let i = 0; i < accounts.length; i++) {
        currAccount = accounts[i];
        try {
          const accountName = currAccount.children[0].children[1].children[0].innerText;
          accountInfomation.channels[accountName] = accounts[i];
        } catch (_) {
        }
      }
      if (setupCurrAccount) {
        config.autoDownVoteCurrChannel = accountInfomation.currChannel;
      }
      // Setup the server
      if (control) {
        setupCVServer();
      }
      if (autoDownVote && runAutoDownVote && urlEqual(window.location.href, url)) {
        delayDownVote();
      }
      // 关闭账号信息页面
    ...
    if (!setuped) {
      accountInfoTimer = setTimeout(setUpAccountInfo, 500);
    }
  };
  accountInfoTimer = setTimeout(setUpAccountInfo, 500);
  ...
  function successCallback(result) {
    // console.log("successCallback: " + result["error"]);
    let newState = "?";
    ...
    if (newState != channelState) {
      ...
      const msg = getAccountInfo();
      console.log(`msg: ${msg}`);
      if (msg && CVServer) {
        CVServer.send(JSON.stringify(msg));
      }
    }
  ...
  const checkSCInfo = () => {
    const { control, autoSCCheck  } = config;
    try {
      if (autoSCCheck) document.getElementById("chatframe").contentWindow.checkSuperChat().then(successCallback, failureCallback);
    } catch (e) {
      // console.log(e);
    }
    if (channelState === "T" || channelState === "?") {
      scCheckTimer = setTimeout(checkSCInfo, 60000);
    } else {
    }
  };
  scCheckTimer = setTimeout(checkSCInfo, 20000);
  // 直播间加载api
  ...
  eventBus.on(
    "dlc.gatlingstop",
    () => {
      refs.autoDownVoteBtn.innerText = "加特林开火";
      eventBus.emit("dlc.stop");
      if (typeof changeChannelTimer !== "undefined") {
        clearTimeout(changeChannelTimer);
      }
      if (typeof delayDownVoteTimer !== "undefined") {
        clearTimeout(delayDownVoteTimer);
      }
    },
    {},
  ...
  eventBus.on(
    "dlc.gatlingrun",
    () => {
      refs.autoDownVoteBtn.innerText = "加特林停火";
      config.url = window.location.href;
      setUpAccountInfo(false, true);
      config.autoDownVoteCurrChannel = accountInfomation.currChannel;
      delayDownVote();
    },
    {},
  ...
  eventBus.on(
    ...
    () => {
      ...
      if (fireMode == 0) {
        ...
        danmakuList = splitter[splitMode](textAmmoPiecing).map((ammo) => refine(ammo));
      }
            ...
            ConfigField(
              {
                ...
                props: { type: "checkbox" },
                valueProp: "checked",
                helpDesc: "勾选后此独轮车进入舰载机模式，由泥头登陆舰:cv-6统一指挥",
              },
            ),
            ConfigField({
              label: "泥头登陆舰指挥端口:",
        ...
        h([
          [
            "button",
            {
              class: "dlc-btn",
              $click: (ev) => {
                config.autoDownVote = !config.autoDownVote;
                if (config.autoDownVote) {
                  eventBus.emit("dlc.gatlingrun");
                } else {
                  eventBus.emit("dlc.gatlingstop");
                }
                localStorage.setItem("duluncheCfg", JSON.stringify(config));
    ...
    "div",
    {
      ...
      $click: () => {
        ...
        if (!tip) {
          tip = true;
          refineJson = getJSON();
          Toast(`广播塔: ${refineJson.announcement}`);
        }
      },
    },
    "初始化中...",
